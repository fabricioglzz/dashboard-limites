...
rango_prom = df_filt["Rango"].mean()
max_rango = df_filt.loc[df_filt["Rango"].idxmax()]
min_rango = df_filt.loc[df_filt["Rango"].idxmin()]

c1, c2, c3 = st.columns(3)

c1.metric(
    "Rango promedio (LSC − LIC)",
    f"{rango_prom:.2f}"
)

c2.metric(
    "Máximo rango",
    f"{max_rango['Rango']:.2f}",
    f"{max_rango[col_base]} – {max_rango[col_var]}"
)

c3.metric(
    "Mínimo rango",
    f"{min_rango['Rango']:.2f}",
    f"{min_rango[col_base]} – {min_rango[col_var]}"
)
